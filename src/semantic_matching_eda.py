...
def main():
    ...
    json_path = os.path.join(
        os.path.dirname(script_filepath),
        os.pardir,
        "revised_su-eatable-life_taxonomy.json",
    )
    ...
    csv_path = os.path.join(
        os.path.dirname(script_filepath), os.pardir, "ingredient_food_kg_names.csv"
    )
    ...
    print(
        "\nTop-k matches ({:.2f} <= cosine similarity < {:.2f}):".format(
            TOPK_THRESHOLD, STRICT_THRESHOLD
        )
    )
    for i, leaf in enumerate(tree_leaves):
        ...
        valid_indices = [
            ...
            if TOPK_THRESHOLD <= score < STRICT_THRESHOLD
        ]
        if not valid_indices:
            continue
        # Sort valid indices by descending score
        valid_indices = sorted(valid_indices, key=lambda j: scores[j], reverse=True)[
            :TOP_K
        ]
    ...
    print(
        "\nTop-k matches ({:.2f} <= cosine similarity < {:.2f}):".format(
            TOPK_THRESHOLD2, TOPK_THRESHOLD
        )
    )
    for i, leaf in enumerate(tree_leaves):
        # Get scores and filter based on threshold
        ...
        valid_indices = [
            idx
            for idx, score in enumerate(scores)
            if TOPK_THRESHOLD2 <= score < TOPK_THRESHOLD
        ]
        if not valid_indices:
            continue
        # Sort valid indices by descending score
