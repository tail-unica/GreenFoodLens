...
parser.add_argument(
    "version_tag", type=str, help="Specify the version tag for the output file"
)
parser.add_argument(
    "--truth_labels_file",
    "-lf",
    help="Specify the path to the labeled ingredients file",
)
parser.add_argument(
    "--context_len",
    "-ctx_len",
    type=int,
    default=0,
    help="Specify the context length for the model",
)
parser.add_argument(
    "--temperature", type=float, default=0, help="Temperature for the LLM"
)
...
parser.add_argument(
    ...
    "-sp_gr",
    action="store_true",
    help="Split the grammar choices into individual characters",
)
parser.add_argument(
    "--verbose", action="store_true", help="Enables llama-cpp verbose mode"
)
parser.add_argument(
    "--use_all_ingredients",
    action="store_true",
    help="Use all ingredients for labeling",
)
parser.add_argument(
    "--validation_split",
    type=float,
    default=0.5,
    help="Validation split for the labeled ingredients",
)
parser.add_argument(
    "--gpu_id", type=int, default=None, help="Specify the GPU ID to use for the LLM"
)
...
if re.match(r"v\d+", args.version_tag) is None:
    raise argparse.ArgumentError(
        "Version tag must be in the format 'vX', where X is an integer"
    )
else:
    version_id = int(args.version_tag.replace("v", ""))

if args.use_all_ingredients:
    ingredients_file = pl.read_csv(
        os.path.join(script_filepath, os.pardir, "ingredient_food_kg_names.csv")
    )
    ingredients = ingredients_file.select(
        pl.col("ingredient_food_kg_names").str.replace('"', "")
    )["ingredient_food_kg_names"].to_list()
else:
    valid_answers_filepath = os.path.join(
        script_filepath, f"{os.path.splitext(args.truth_labels_file)[0]}_valid.csv"
    )
    test_answers_filepath = os.path.join(
        script_filepath, f"{os.path.splitext(args.truth_labels_file)[0]}_test.csv"
    )
    if not os.path.exists(valid_answers_filepath) or not os.path.exists(
        test_answers_filepath
    ):
    ...
    ingredients = (
        ...
        .select(pl.col("ingredient").str.replace('"', ""))["ingredient"]
        .to_list()
    )

output_path = os.path.join(
    script_filepath, "LLM Ingredient Labeling", "llama_cpp_grammar"
)
...
with open(
    os.path.join(script_filepath, os.pardir, "revised_su-eatable-life_taxonomy.json"),
    "r",
) as f:
...
if args.gpu_id is not None:
    gpu_kwargs = dict(split_mode=llama_cpp.LLAMA_SPLIT_MODE_NONE, main_gpu=args.gpu_id)
else:
    gpu_kwargs = dict()
...
gen_params = dict(temperature=args.temperature, top_p=args.top_p, top_k=args.top_k)
...
if version_id > 1:
    system_messages.append(
        {"role": "system", "content": prompt_templates["labeling_notes"]}
    )
...
for i in tqdm.tqdm(range(len(ingredients)), desc="Ingredients Labeling"):
    ...
    boostrap_messages = [
        *system_messages,
        {
            "role": "user",
            "content": prompt_templates["bootstrap_description"].format(
                ingredient=ingr
            ),
        },
    ]

    bootstrap_llm_output = llm.create_chat_completion(
        boostrap_messages, max_tokens=200, **gen_params
    )
    labeler_messages = [
        *boostrap_messages,
    ...
    while True:
        partial_path_messages = [
            *labeler_messages,
            {
                "role": "user",
                "content": prompt_templates["instruction_with_candidates"].format(
                    ingredient=ingr,
                    partial_path_instruction=prompt_templates[
                        "partial_path_instruction"
                    ].format(path=partial_path),
        ...
        path_candidates_grammar = llama_cpp.LlamaGrammar.from_string(
            path_candidates_grammar
        )

        partial_path_output = llm.create_chat_completion(
            partial_path_messages, grammar=path_candidates_grammar, **gen_params
        )
        generated_path_node = partial_path_output["choices"][0]["message"][
            "content"
        ].replace("`", "")

        assert generated_path_node in path_candidates, (
            f"llama-cpp grammar not working as expected, generated path node: {generated_path_node}, path candidates: {path_candidates}"
        )
        if "I DON'T KNOW" in generated_path_node:
            ...
            path_candidates = list(ingredient_tree[generated_path_node]["children"]) + [
                "I DON'T KNOW"
            ]
        else:
            partial_path += generated_path_node
    ...
    print(
        f"Time taken: {int(elapsed_time // 3600):02d}:{int((elapsed_time % 3600) // 60):02d}:{int(elapsed_time % 60):02d}"
    )
    labeled_ingredients.append((i, ingr, generated_path))

    pl.DataFrame(
        labeled_ingredients, schema=["index", "ingredient", "answer_path"]
