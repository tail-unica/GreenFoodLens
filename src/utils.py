import re
from typing import Dict, Set, Literal, Union
...
def ingredient_tree_from_json(json_ingredient_data):
    ...
    assert len(all_labels) == len(set(all_labels)), (
        "Duplicate labels found in json ingredient data"
    )
    ...
    for ingredient_dict in json_ingredient_data:
        ...
        if ingredient.label not in ingredient_tree:
            ingredient_tree[ingredient.label] = {
...
def traverse_ingredients(
    ingr_parent_child: Dict[str, Union[str, Set[str]]],
    ...
    for i, child in enumerate(starting_nodes):
        ...
        if not ingr_info["children"]:
            ...
            path_string += " -> "
            path_string = traverse_ingredients(
                ingr_parent_child,
                starting_nodes=ingr_info["children"],
...
def clean_and_save_llm_output(llm_output_filepath, output_csv_filepath):
    ...
    cleaned_labeled_ingredients_df = pl.DataFrame(
        cleaned_labeled_ingredients, schema=["ingredient", "answer_path"]
    )
    cleaned_labeled_ingredients_df.write_csv(output_csv_filepath, separator="\t")

    print(
        f"Labeled ingredients after cleaning LLM output saved to {output_csv_filepath}"
    )
...
def tree_to_pydantic_schema(tree):
    ...
    def traverse_tree_to_pydantic(starting_nodes: Set[str]):
        ...
        for node_name in starting_nodes:
            ...
            leaf_children = [
                leaf for leaf in node_info["children"] if not tree[leaf]["children"]
            ]
            non_leaf_children = [
                leaf for leaf in node_info["children"] if tree[leaf]["children"]
            ]

            # leaf_enum = Enum(f"{node_name}-Items", {leaf: leaf for leaf in leaf_children})
            leaf_enum = (Literal[leaf] for leaf in leaf_children)
            non_leaf_schemas = traverse_tree_to_pydantic(
                starting_nodes=non_leaf_children
            )

            node_schema = pydantic.create_model(
                node_name,
                next_node=(Union[(*leaf_enum, i_dont_know, *non_leaf_schemas)], ...),
            )
            children_schemas.append(node_schema)

        return children_schemas

    starting_nodes_schemas = traverse_tree_to_pydantic(
        starting_nodes=tree["-"]["children"]
    )

    full_schema = pydantic.create_model(
        "SU-EATABLE-Path",
        starting_node=(Union[(i_dont_know, *starting_nodes_schemas)], ...),
    )
        ...
        if split_chars:
            ...
            grammar.append(
                [
                    ...
                    assign_op,
                    *[
                        '"' + choice_chars[j] + '"' if (j % 2) == 0 else choice_op
                        for j in range(len(choice_chars) * 2 - 1)
                    ],
                ]
            )
        else:
